...
Para catalogar todos los artículos académicos, confiamos en la base de datos [Crossref](https://www.crossref.org/). Crossref es una agencia de registro de DOI (una entidad capaz de asignar DOI) para publicaciones académicas [@doi:10.6087/kcse.2014.1.13]. Actualmente hay 10 agencias de registro. [Estimamos](https://github.com/greenelab/crossref/issues/3) que Crossref ha registrado el 67% de todos los DOI existentes. Si bien varias agencias de registro asignan DOI a publicaciones académicas, Crossref es el principal registrador. En marzo de 2015, de los 1,464,818 enlaces de DOI válidos en la versión en inglés de Wikipedia, el 99.9% estaban registrados en Crossref [@doi:10.1007/978-3-319-49304-6_40]. Este porcentaje fue ligeramente inferior para otros idiomas: 99.8% en Wikipedia en chino y 98.0% en Wikipedia en japonés. Por lo tanto, la abrumadora mayoría de los artículos académicos referenciados por DOI están registrados en Crossref. Dado que Crossref tiene el acceso programático más exhaustivo y completo, tuvimos un fuerte incentivo para centrarnos únicamente en los DOI registrados por Crossref. Dada la preeminencia de Crossref, es poco probable que la omisión de otras agencias de registro influya sustancialmente en nuestros hallazgos.
...
Consultamos el endpoint de `works` de la [Crossref API](https://api.crossref.org/) para recuperar los metadatos de todos los DOI, almacenando las respuestas en una base de datos MongoDB. Las consultas comenzaron el 21 de marzo de 2017 y tardaron 12 días en completarse. En total, recuperamos metadatos para 87,542,370 DOI, correspondientes a todos los trabajos de Crossref a partir del 21 de marzo de 2017. El código fuente de este paso está disponible en GitHub en [greenelab/crossref](https://github.com/greenelab/crossref). Debido al gran tamaño del archivo (7,4 GB), la exportación de la base de datos MongoDB de metadatos DOI no está disponible en GitHub, y en su lugar se aloja a través de figshare [@doi:10.6084/m9.figshare.4816720.v1]. Creamos archivos TSV con la información mínima necesaria para este estudio: Primero, una tabla DOI con columnas para el tipo de trabajo y la fecha de emisión. La fecha de emisión se refiere a la fecha de publicación más antigua conocida, es decir, la fecha de publicación impresa o en línea, lo que ocurra primero. En segundo lugar, un mapeo de DOI a ISSN para asociar artículos con su revista de publicación.
...
[Seleccionamos](https://github.com/greenelab/scihub/issues/7) un subconjunto de tipos de trabajo Crossref para incluir en nuestros análisis de cobertura de Sci-Hub que correspondían a artículos académicos (es decir, publicaciones). Como no pudimos localizar definiciones para los tipos Crossref, utilizamos nuestro mejor criterio y evaluamos trabajos de muestra de un tipo dado en caso de incertidumbre. Incluimos los siguientes tipos: `capítulo de libro`, `parte de libro`, `sección de libro`, `artículo de revista`, `artículo de actas de congresos`, `entrada de referencia`, `reporte` y `estándar`. Se excluyeron tipos como `libros`, `revistas`, `números de revistas` y `series de reportes`, ya que generalmente son contenedores para artículos individuales en lugar de artículos académicos per se. Después de filtrar por tipo, quedaron 81,609,016 DOI (77,201,782 de los cuales tenían su año de publicación disponible). Para los propósitos de este estudio, estos DOI representan la totalidad de la literatura académica.
