...
Este manuscrito
([permalink](https://greenelab.github.io/scihub-manuscript-es/v/d784de5f51663167c08281380906f8ddf398c70f/))
fue generado automaticamente
de [greenelab/scihub-manuscript-es@d784de5](https://github.com/greenelab/scihub-manuscript-es/tree/d784de5f51663167c08281380906f8ddf398c70f)
...
### Catálogo de artículos académicos derivado de Crossref

Para catalogar todos los artículos académicos, confiamos en la base de datos [Crossref](https://www.crossref.org/). 
Crossref es una agencia de registro de DOI (una entidad capaz de asignar DOI) para publicaciones académicas [@Eq78IAX4]. 
Actualmente hay 10 agencias de registro. 
[Estimamos](https://github.com/greenelab/crossref/issues/3) que Crossref ha registrado el 67% de todos los DOI existentes. 
Si bien varias agencias de registro asignan DOI a publicaciones académicas, Crossref es el principal registrador. 
En marzo de 2015, de los 1,464,818 enlaces de DOI válidos en la versión en inglés de Wikipedia, el 99.9% estaban registrados en Crossref [@aUCPtxQc]. 
Este porcentaje fue ligeramente inferior para otros idiomas: 99.8% en Wikipedia en chino y 98.0% en Wikipedia en japonés. 
Por lo tanto, la abrumadora mayoría de los artículos académicos referenciados por DOI están registrados en Crossref. 
Dado que Crossref tiene el acceso programático más exhaustivo y completo, tuvimos un fuerte incentivo para centrarnos únicamente en los DOI registrados por Crossref. 
Dada la preeminencia de Crossref, es poco probable que la omisión de otras agencias de registro influya sustancialmente en nuestros hallazgos.
...
Consultamos el endpoint de `works` de la [Crossref API](https://api.crossref.org/) para recuperar los metadatos de todos los DOI, almacenando las respuestas en una base de datos MongoDB. 
Las consultas comenzaron el 21 de marzo de 2017 y tardaron 12 días en completarse. 
En total, recuperamos metadatos para 87,542,370 DOI, correspondientes a todos los trabajos de Crossref a partir del 21 de marzo de 2017.
El código fuente de este paso está disponible en GitHub en [greenelab/crossref](https://github.com/greenelab/crossref). 
Debido al gran tamaño del archivo (7,4 GB), la exportación de la base de datos MongoDB de metadatos DOI no está disponible en GitHub, y en su lugar se aloja a través de figshare [@1E6E4mpZ]. 
Creamos archivos TSV con la información mínima necesaria para este estudio: 
Primero, una tabla DOI con columnas para el tipo de trabajo y la fecha de emisión. 
La fecha de emisión se refiere a la fecha de publicación más antigua conocida, es decir, la fecha de publicación impresa o en línea, lo que ocurra primero. 
En segundo lugar, un mapeo de DOI a ISSN para asociar artículos con su revista de publicación.
...
[Seleccionamos](https://github.com/greenelab/scihub/issues/7) un subconjunto de tipos de trabajo Crossref para incluir en nuestros análisis de cobertura de Sci-Hub que correspondían a artículos académicos (es decir, publicaciones). 
Como no pudimos localizar definiciones para los tipos Crossref, utilizamos nuestro mejor criterio y evaluamos trabajos de muestra de un tipo dado en caso de incertidumbre. 
Incluimos los siguientes tipos: `capítulo de libro`, `parte de libro`, `sección de libro`, `artículo de revista`, `artículo de actas de congresos`, `entrada de referencia`, `reporte` y `estándar`. 
Se excluyeron tipos como `libros`, `revistas`, `números de revistas` y `series de reportes`, ya que generalmente son contenedores para artículos individuales en lugar de artículos académicos per se. 
Después de filtrar por tipo, quedaron 81,609,016 DOI (77,201,782 de los cuales tenían su año de publicación disponible). 
Para los propósitos de este estudio, estos DOI representan la totalidad de la literatura académica.
...
### Catálogo de artículos de Sci-Hub

El 19 de marzo de 2017, Sci-Hub [tuiteó](https://twitter.com/Sci_Hub/status/843546352219017218): 
"Si le interesa la lista de todos los DOI recopilados en Sci-Hub, aquí está: sci-hub.cc/downloads/doi.7z ... 62,835,101 DOI en orden alfabético". 
El tweet incluyó un enlace de descarga para un archivo con los 62,835,101 DOIs a los que Sci-Hub dice que proporciona acceso. 
De estos DOI, 56,246,220 eran parte del catálogo de artículos académicos derivado de Crossref, y el 99.5% de los DOI de la lista de Sci-Hub estaban en el repositorio de scimag LibGen (después del filtrado). 
Por lo tanto, los depósitos de Scimag y Sci-Hub de LibGen se han mantenido sincronizados en gran medida desde su separación. 
En Twitter, la cuenta Sci-Hub confirmó este hallazgo, comentando "con pequeñas diferencias, sí, la base de datos es la misma". 
Por lo tanto, los catálogos LibGen scimag y Sci-Hub DOI se pueden usar de manera intercambiable para fines de investigación.
...
### Estado de conjuntos de datos de OA

[oaDOI](https://oadoi.org/), abreviatura de DOI de acceso abierto, es un servicio que determina si un DOI está disponible de forma gratuita en algún lugar en línea [@GhX9kmM1]. 
oaDOI no indexa artículos publicados en redes sociales académicas o disponibles en repositorios ilícitos como Sci-Hub [@eOi5pOoT]. 
Utilizando la infraestructura de oaDOI, el estudio del estado de OA investigó la disponibilidad de artículos de tres colecciones ([@eOi5pOoT]. 
Cada colección consta de una muestra aleatoria de aproximadamente 100,000 artículos de un corpus más grande. 
Describimos las colecciones a continuación e informamos el número de artículos después de la intersección con nuestro catálogo DOI:

+ **Web of Science**: 103,491 artículos publicados entre 2009–2015 y clasificados como elementos citables en Web of Science.
+ **Unpaywall**: 87,322 artículos visitados por usuarios de Unpaywall del 5 al 11 de junio de 2017.
+ **Crossref**: 99,952 artículos del tipo de `artículo de revista` Crossref.

[Unpaywall](http://unpaywall.org/) es una extensión de navegador web que notifica a su usuario si un artículo está disponible a través de oaDOI [@8AIp1E7j]. 
Dado que la colección Unpaywall se basa en artículos que los usuarios visitaron, es un mejor reflejo de las necesidades reales de acceso de los académicos contemporáneos. 
Desafortunadamente, dado que el número de visitas por artículo no se conserva en este conjunto de datos, las estimaciones de la tasa de cumplimiento están sesgadas en relación con los artículos muy visitados y se convierten en una variante de escala (afectada por la popularidad de Unpaywall).
...
El estudio del estado de OA determinó el estado de accesibilidad de cada DOI en cada colección utilizando oaDOI [@eOi5pOoT; @1D9l2pC2b]. 
Los artículos para los cuales oaDOI no identificó un texto completo se consideraron "cerrados". 
De lo contrario, a los artículos se les asignó un color/estado de bronce, verde, híbrido u oro. 
oaDOI clasifica los artículos que no están disponibles en el sitio de su editorial como verdes o cerrados. 
La versión de oaDOI utilizada en el estudio del estado de OA identificó artículos verdes al buscar en PubMed Central y <abbr title="Bielefeld Academic Search Engine">BASE</abbr>. 
Los lectores deben tener en cuenta que esta implementación [probablemente cuenta menos](https://github.com/greenelab/scihub-manuscript/issues/36) que los artículos verdes, especialmente si consideran los artículos disponibles de las redes sociales académicas como verdes.
...
### Catálogo de citas recientes

OpenCitations es un recurso de dominio público que contiene datos de citas académicas [@H2hXqKsr]. 
OpenCitations extrae su información del subconjunto de acceso abierto de PubMed Central. 
En el repositorio de [greenelab/opencitations](https://github.com/greenelab/opencitations), procesamos la publicación de datos OpenCitations del 25 de julio de 2017 [@n73t9GPC; @10hS1L2vZ; @1EcDRAPGi], creando un catálogo de referencias bibliográficas DOI – citas – DOI. 
Para el control de calidad, eliminamos los DOI que no formaban parte del catálogo de artículos derivados de Crossref. 
Además, eliminamos las citas salientes de los artículos publicados antes de 2015. 
Las citas entrantes de artículos anteriores a 2015 no se eliminaron. 
El catálogo resultante consistió en 7,312,607 citas de 200,206 artículos recientes a 3,857,822 artículos referenciados.
...
El estudio de 2016 titulado "¿Quién está descargando documentos pirateados? Todos ” analizó un conjunto de datos de registros de acceso de Sci-Hub [@RsNXQAG2; @t4r48QpO]. 
Alexandra Elbakyan trabajó con el periodista John Bohannon para producir un conjunto de datos de las solicitudes resueltas de Sci-Hub desde el 1 de septiembre de 2015 hasta el 29 de febrero de 2016 [@kQFQ8EaO]. 
En noviembre de 2015, el nombre de dominio de Sci-Hub fue suspendido como resultado de una acción legal de Elsevier [@16k28dBmH; @sR4uAYxB]. 
Según Bohannon, esto resultó en "una brecha de 18 días en los datos a partir del 4 de noviembre de 2015 cuando el dominio sci-hub.org se cayó y los registros del servidor se configuraron incorrectamente". 
Mostramos este tiempo de inactividad en la Figura @fig:history.
...
Filtramos los eventos de acceso excluyendo los DOI no incluidos en nuestro catálogo de literatura y omitiendo los registros que ocurrieron antes de la fecha de publicación de un artículo. 
Este filtro conservó 26,984,851 eventos de acceso para 10,293,836 DOI distintos (97.5% de los 10,552,418 DOI pre filtrados distintos).
Resumimos los eventos de acceso para cada artículo utilizando las siguientes métricas:

1. descargas: número total de veces que se accedió al artículo
2. visitantes: número de direcciones IP que accedieron al artículo
3. países: número de países (geolocalización por dirección IP) desde los que se accedió al artículo
4. días: número de días en que se accedió al artículo
5. meses: número de meses en que se accedió al artículo
...
A continuación, calculamos las métricas de acceso a nivel de revista en base a los artículos publicados desde el 1 de enero de 2014 hasta el inicio de los registros de acceso de Sci-Hub el 1 de septiembre de 2015. 
Para cada revista, calculamos los valores promedio para las cinco métricas de registro de acceso descrito arriba. 
Curiosamente, la revista _Medicina - Programa de Formación Médica Continuada Acreditado_ [recibió](https://greenelab.github.io/scihub/#/journal/21100213100 "Sci-Hub Stats Browser page for the journal Medicine") la mayor cantidad de visitantes por artículo, con un promedio de 33.4 visitantes por cada uno de sus 326 artículos.
...
Tenga en cuenta que estos análisis no incluyen los registros de acceso de Sci-Hub para 2017 [@D6wP8q7K], que se [publicaron](https://twitter.com/Sci_Hub/status/954095639859814402) el 18 de enero de 2018. 
Desafortunadamente, en ese momento ya habíamos adoptado un congelamiento de principales análisis nuevos. 
No obstante, hicimos un análisis rápido para evaluar el crecimiento de las descargas de Sci-Hub a lo largo del tiempo que combinó los datos de registro de acceso de 2015-2016 y 2017 (Figura @fig:downloads).
...
## Figuras Supplementarias

![
**Descargas por día en Sci-Hub durante meses con registros de acceso.**
El número de artículos descargados de Sci-Hub se muestra en el tiempo.
Los registros de acceso de Sci-Hub se combinaron a partir de dos versiones: @kQFQ8EaO que abarca 27.819.963 descargas de septiembre de 2015 a febrero de 2016 y @D6wP8q7K que abarca 150.875.862 descargas de 2017.
El gráfico muestra el número promedio de descargas por día durante meses con datos.
Hubo 54 días dentro de los períodos de recolección sin ningún evento de acceso registrado, debido presumiblemente a interrupciones del servicio o configuración incorrecta del servidor.
Por lo tanto, ignoramos los días sin registros al calcular los promedios mensuales.
El color del punto indica la proporción de días con registros para un mes determinado.
Por ejemplo, noviembre de 2015 y octubre de 2017, a los que les faltaban registros durante 17 y 23 días, respectivamente, son más ligeros.
La caída de las descargas de diciembre de 2017 probablemente refleja el efecto de las suspensiones de dominio que ocurrieron a fines de noviembre [@pmrIfHRq].
A diferencia de los análisis de registro de Sci-Hub en otras partes de este estudio, este gráfico no filtra por artículos válidos (es decir, DOI en nuestro catálogo de literatura académica derivado de Crossref)
...
![
**Cobertura por país de publicación.**
Scopus asigna a cada revista un país de publicación.
La cobertura de Sci-Hub se muestra para países con al menos 100,000 artículos.
...
![
**Cobertura de artículos por año publicado y estado de acceso a la revista.**
La cobertura de Sci-Hub se muestra por separado para artículos en revistas en acceso abierto versus de pago, para cada año desde 1950.
...
![
**Cobertura por estado de acceso de oaDOI en cada estado de la colección de OA.**
La cobertura por el estado de acceso de oaDOI se muestra para Sci-Hub, PennText y la unión de Sci-Hub y PennText.
Cada panel se refiere a una colección diferente de estado de OA, y Combined se refiere a la unión de las colecciones Crossref, Unpaywall y Web of Science.
La sección Sci-Hub del panel Combinado es la misma que la Figura @fig:oadoi.
Impresionantemente, la cobertura de Sci-Hub de los artículos cerrados en la colección de Web of Science fue del 97.8%.
Es probable que esta notable cobertura refleje que estos artículos fueron publicados entre 2009 y 2015 y que Web of Science los clasificó como elementos citables, que es selectivo al indexar revistas [@QyHSyHo3].
Tenga en cuenta que PennText no tiene una cobertura completa de artículos de bronce, híbridos y oro, lo que debería ser el caso si todos los sistemas de metadatos fueran perfectos.
Es probable que estas anomalías resulten de errores tanto en PennText (cuya precisión estimamos en 88.2%) como en oaDOI (cuya precisión del estudio del Estado de OA estimó en 90.4%, es decir, la Tabla 1 de @eOi5pOoT informa 5 falsos positivos y 43 falsos negativos en detecciones oaDOI para 500 artículos).
...
![
**Donaciones de Bitcoin a Sci-Hub por mes.**
Durante meses desde junio de 2015, se evaluaron las donaciones totales de bitcoins (depósitos a direcciones conocidas de Sci-Hub).
Las donaciones en USD se refieren al valor en dólares de los Estados Unidos al momento de la confirmación de la transacción.
...
![
**Tiempo de retraso desde la publicación hasta la carga en LibGen.**
Para cada año de publicación desde 2010-2017, trazamos la relación entre el tiempo de retraso y la cobertura de LibGen scimag.
Por ejemplo, este gráfico muestra que el 75% de los artículos publicados en 2011 se cargaron en LibGen dentro de los 60 meses.
Este análisis solo considera artículos para los que se puede extraer de manera confiable un mes de publicación, lo que excluye todos los artículos supuestamente publicados el 1 de enero.
Este gráfico retrata los tiempos de retraso como decrecientes con el tiempo, con una cobertura máxima en declive.
Por ejemplo, la cobertura de los artículos de 2016 excedió el 50% en 6 meses, pero parece haber alcanzado una asíntota de alrededor del 60%.
Alternativamente, la cobertura para 2014 tardó 15 meses en superar el 50%, pero desde entonces ha alcanzado el 75%.
Sin embargo, esta señal podría ser el resultado de las marcas de tiempo de carga de LibGen posteriores a la fecha.
Por lo tanto, advertimos en no sacar conclusiones del campo `TimeAdded` en LibGen scimag hasta que su precisión pueda establecerse de manera más confiable.
